...
# =========================
SHEET_ID = "1clzNg0YblSQVvpWlWqeAwHKYiTyKcv-meWaI1RILAFo"
SHEET_GID = "0"
SHEET_EXPORT_URL = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/export?format=xlsx&gid={SHEET_GID}"


# =========================
# Mapeo ID -> Nombre de máquina
# =========================
MACHINE_NAME_TO_ID = {
...
def normalizar_columnas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ✅ FIX MINIMO: si vienen columnas duplicadas, Pandas devuelve DataFrame al acceder por nombre.
    # Nos quedamos con la primera ocurrencia para que df["Id Equipo"] sea Serie.
    ...
    _fecha_col = df["Fecha"]
    if isinstance(_fecha_col, pd.DataFrame):
        _fecha_col = _fecha_col.iloc[:, 0]
    df["Fecha"] = pd.to_datetime(_fecha_col, errors="coerce", infer_datetime_format=True)

    _id_equipo_col = df["Id Equipo"]
    if isinstance(_id_equipo_col, pd.DataFrame):
        _id_equipo_col = _id_equipo_col.iloc[:, 0]
    df["Id Equipo"] = _id_equipo_col.astype(str).str.strip()
...
if not modo_multiple_maquinas:
    ...
    maquinas_pick = col_top1.multiselect(
        "Máquinas",
        opciones_maquina,
        default=nombres_disponibles[:1] if nombres_disponibles else []
    )
...
def render_dia(df_base: pd.DataFrame, maquina_id: str, maquina_nombre: str, fecha_dia: date, umbral_min: int):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        worksheet.set_column(2, 2, 12, time_fmt)
    ...
    for maquina_nombre, maquina_id in maquinas_seleccionadas:
        ...
        if len(resumen) > 1:
            ...
            for i, y in enumerate(df_res["%_Perdido"]):
                ax.text(i, y + bump, f"{y:.2f}%", ha="center", va="bottom", fontsize=7, fontweight="bold")
